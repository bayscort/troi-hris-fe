...
import { receiptService, accountService } from "../services/api";
...
const ReceiptPage: React.FC = () => {
    ...
    const currentReceipts = receipts.slice((currentPage - 1) * perPage, currentPage * perPage);
    ...
    const fetchReceipts = async () => {
        ...
        try {
            ...
            const data = await receiptService.getAllReceipts(accountId, startDate, endDate);
            setReceipts(data);
            setCurrentPage(1);
        } catch (error) {
    ...
    const generatePageForBatch = (doc: jsPDF, batch: ReceiptDTO[], receiptDate: string, accountType: string, accountName: string) => {
        ...
        const drawHeader = () => {
            ...
            doc.text('BUKTI PENERIMAAN', pageWidth / 2, 12, { align: 'center' });
    ...
    const handleDownloadPdf = async () => {
        ...
        try {
            const groupedData: GroupedReceipt[] = await receiptService.getAllGrouped(
                accountId,
                dateRange.startDate,
                dateRange.endDate
            );
            ...
            groupedData.forEach(group => {
                group.batches.forEach(batch => {
                    if (batch.length > 0) {
                        ...
                        generatePageForBatch(doc, batch, group.receiptDate, batch[0].account.accountType, batch[0].account.name);
                        isFirstBatch = false;
                    }
                });
            });
            ...
            doc.save(`Bukti_Penerimaan_${accountName}_${startDate}_sampai_${endDate}.pdf`);
    ...
    const handleSuccess = (action: 'created' | 'updated') => {
        ...
        setNotification({ message: `Receipt successfully ${action}.`, type: 'success' });
    };

    const handleEditClick = (receipt: ReceiptDTO) => {
        setSelectedReceipt(receipt);
        setIsEditModalOpen(true);
    };

    const handleDeleteClick = (receipt: ReceiptDTO) => {
        setSelectedReceipt(receipt);
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            setNotification({ message: 'Receipt deleted successfully.', type: 'success' });
            setRefreshTrigger(x => x + 1);
        } catch (e) {
            setNotification({ message: 'Failed to delete receipt.', type: 'error' });
            console.error(e);
        } finally {
    ...
    return (
        <div className="bg-white min-h-screen font-sans">
                            ...
                            {loading ? (
                                ...
                                currentReceipts.map((r) => (
                                    <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                                        <td className="px-6 py-4 whitespace-nowrap">{format(new Date(r.receiptDate), "dd MMM yyyy")}</td>
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex justify-center items-center gap-2">
                                                {hasPermission('receipt', 'READ') && (
                                                    <button
                                                        onClick={() => { setSelectedReceipt(r); setIsDetailModalOpen(true); }}
                ...
                {receipts.length > 0 && (
                    <div className="flex justify-between items-center text-sm">
                        <span>
                            ...
                            {Math.min(currentPage * perPage, receipts.length)} of {receipts.length}
                        </span>
            ...
            {isEditModalOpen && selectedReceipt && (
                <ReceiptForm
                    receiptToEdit={{
                        ...
                        accountId: selectedReceipt.account?.id,
                        financeItemId: selectedReceipt.financeItem!.id,
                        amount: selectedReceipt.amount,
                        reconciled: selectedReceipt.reconciled,
            ...
            <DeleteConfirmationModal
                ...
                itemName={`receipt for ${selectedReceipt?.financeItem?.name || ''} on ${selectedReceipt ? format(new Date(selectedReceipt.receiptDate), "dd MMM yyyy") : ''}`}
