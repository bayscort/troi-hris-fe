...
import ReceiptSummaryTable from '../components/report/ReceiptSummaryTable';
...
export default function ReceiptReport() {
  ...
  const onExport = () => {
    ...
    XLSX.writeFile(workbook, `LaporanPemasukan_${fileStartStr}-${fileEndStr}.xlsx`, {
      cellStyles: true,
      sheetStubs: false
    });
  };
  ...
  useEffect(() => {
    const loadReportData = async () => {
      ...
      try {
        ...
        const data = await fetchFinanceReport('receipt', selectedAccountId, startDateStr, endDateStr);
        ...
        {selectedAccountId ? (
          <section className="space-y-6">
            <SectionHeader
              ...
              description="Rincian data penerimaan berdasarkan filter yang dipilih."
            />
            <ReceiptSummaryTable categories={reportData} loading={loading} />
          </section>
        ) : (
          <ReportPlaceholder />
        )}
