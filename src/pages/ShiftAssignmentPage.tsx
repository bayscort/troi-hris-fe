...
import {
    ...
    ArrowRight, Loader, AlertCircle, Upload, FileSpreadsheet, X
} from 'lucide-react';
...
const ShiftAssignmentPage: React.FC = () => {
    ...
    const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            if (fileExtension === 'xlsx' || fileExtension === 'xls' || validTypes.includes(file.type)) {
    ...
    // Handler for manual schedule upload
    const handleUploadManualSchedule = async () => {
        if (!selectedFile) return alert("Please select a file");

        setUploading(true);
        try {
            await assignmentShiftService.uploadManualSchedule(selectedFile);
            ...
            alert('Failed to upload schedule. Please check the file format and try again.');
        } finally {
            setUploading(false);
        }
    };
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden">
                        <div className="px-6 py-4 border-b bg-gray-50 flex items-center justify-between">
                            <div>
                                <h3 className="font-semibold text-lg">Import Manual Schedule</h3>
                                <p className="text-sm text-gray-500">Upload Excel file to import shift schedule</p>
                            </div>
                        ...
                        <div className="p-6 space-y-4">
                            {/* File Upload Area */}
                            <div
                                className={`border-2 border-dashed rounded-lg p-6 text-center transition-colors ${selectedFile ? 'border-green-400 bg-green-50' : 'border-gray-300 hover:border-[#ff6908]'
                                    }`}
                            >
                                <input
                                    ref={fileInputRef}
                                ...
                                {selectedFile ? (
                                    ...
                                    <label htmlFor="excel-file-input" className="cursor-pointer block">
                                        ...
                                        <p className="font-medium text-gray-700">Click to select Excel file</p>
                            ...
                            <div className="flex gap-2 p-3 bg-blue-50 text-blue-800 text-xs rounded border border-blue-100">
                                <AlertCircle size={14} className="shrink-0 mt-0.5" />
                                <span>Make sure the Excel file follows the required format. Existing schedules may be overwritten.</span>
                            </div>
                        </div>

                        <div className="px-6 py-4 bg-gray-50 flex justify-end gap-3 border-t">
                            <button onClick={handleCloseImportModal} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded">Cancel</button>
                            <button
                                onClick={handleUploadManualSchedule}
