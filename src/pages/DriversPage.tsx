...
const DriversPage: React.FC = () => {
  ...
  const currentDrivers = filteredDrivers.slice((currentPage - 1) * perPage, currentPage * perPage);
    ...
    <div className="p-8 flex-1 overflow-auto bg-white text-gray-800 font-sans">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={4} className="text-center py-6 text-gray-500">No drivers found.</td></tr>
            ) : (
                ...
                <tr key={d.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 flex justify-end gap-2">
                    {hasPermission('driver', 'READ') && (
                      <button onClick={() => { setSelectedDriver(d); setIsDetailOpen(true); }} className="text-gray-500 hover:text-black">
                        <Eye size={16} />
                      </button>
                    )}
                    {hasPermission('driver', 'UPDATE') && (
                      <button onClick={() => { setSelectedDriver(d); setIsEditModalOpen(true); }} className="text-blue-600 hover:text-blue-800">
                        <Pencil size={16} />
                      </button>
                    )}
      ...
      {filteredDrivers.length > 0 && (
        <div className="flex justify-between items-center text-sm">
          <span>Showing {(currentPage - 1) * perPage + 1}–{Math.min(currentPage * perPage, filteredDrivers.length)} of {filteredDrivers.length}</span>
      ...
      {isCreateModalOpen && (
        <DriverForm isOpen onClose={() => setIsCreateModalOpen(false)} onSave={handleSave} />
      )}
      {isEditModalOpen && selectedDriver && (
        <DriverForm isOpen onClose={() => setIsEditModalOpen(false)} onSave={handleSave} driver={selectedDriver} />
      )}
