...
import { accountService, financeItemService, expenditureService } from "../../services/api";
...
const ExpenditureForm: React.FC<ExpenditureFormProps> = ({ onClose, onSuccess, expenditureToEdit }) => {
  ...
  const isEditMode = !!expenditureToEdit;

  useEffect(() => {
    accountService.getAllAccounts().then(setAccounts);
    financeItemService.getAllFinanceItems().then(setFinanceItems);

    if (isEditMode && expenditureToEdit) {
      setDate(new Date(expenditureToEdit.expenditureDate).toISOString().split('T')[0]);
      setAccountId(String(expenditureToEdit.accountId));
      setItems([{
        financeItemId: String(expenditureToEdit.financeItemId),
        // Format amount awal dari data
        amount: expenditureToEdit.amount.toLocaleString('id-ID', { maximumFractionDigits: 10 }),
        isReconciled: expenditureToEdit.reconciled,
        note: expenditureToEdit.note
      }]);
    }

  }, [expenditureToEdit, isEditMode]);
  ...
  const updateItem = (index: number, field: keyof ExpenditureItem, value: string | boolean) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode && expenditureToEdit) {
        ...
        await expenditureService.updateExpenditure(expenditureToEdit.id, payload);
      } else {
        ...
        await expenditureService.createExpenditures(payload);
      }

      onSuccess();
    } catch (error) {
      console.error("Failed to save expenditure(s):", error);
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200 bg-gray-50 flex-shrink-0">
          <h2 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
            {isEditMode ? 'Edit Expenditure' : 'Add New Expenditures'}
          </h2>
