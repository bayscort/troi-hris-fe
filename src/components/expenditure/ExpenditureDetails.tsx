...
const ExpenditureDetails: React.FC<ExpenditureDetailsProps> = ({ expenditure, onClose }) => {
  const amount = expenditure.amount ?? 0;
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex justify-center items-center z-50 p-4">
      <motion.div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Expenditure details"
      >
          ...
          <div className="flex flex-col md:flex-row gap-6">
            <div className="md:w-1/3">
              <div className="bg-gray-50 border border-gray-100 rounded-xl p-4 flex flex-col items-center">
                ...
                <div className="mt-3 w-full text-center text-xs text-gray-500">
                  <div>{expenditure.financeItem?.name || "-"}</div>
                </div>
              </div>
            </div>
            ...
            <div className="md:flex-1">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <DetailRow label="Finance Item" value={expenditure.financeItem?.name || "-"} />
                <DetailRow label="Account" value={expenditure.account?.name || "-"} />
                <DetailRow label="Date" value={format(new Date(expenditure.expenditureDate), "dd MMM yyyy")} />
                <DetailRow label="Note" value={expenditure.note ||  ""} />
              </div>
            </div>
          </div>
        </div>
