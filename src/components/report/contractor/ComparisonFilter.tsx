...
export default function ComparisonFilter({ contractors, onFilterChange }: ComparisonFilterProps) {
  const [selectedContractors, setSelectedContractors] = useState<ContractorOption[]>([]);
  ...
  const contractorOptions: ContractorOption[] = contractors
    .filter(contractor => contractor.id !== undefined)
    .map(contractor => ({
      value: contractor.id as number,
      label: contractor.name,
    }));

  const handleContractorChange = (selectedOptions: any) => {
    const options = selectedOptions as ContractorOption[];
    setSelectedContractors(options);
  ...
  const handleDateChange = (ranges: any) => {
    ...
    if (startDate && endDate) {
      onFilterChange(startDate, endDate, selectedContractors.map(v => v.value));
      setShowPicker(false);
    }
  };
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm mb-6">
      <div className="grid grid-cols-1 lg:grid-cols-[auto_1fr_auto] gap-4 items-end">
        ...
        <div className="w-full">
          <label className="text-sm font-medium text-gray-700 mb-1 block">Select Contractors to Compare</label>
