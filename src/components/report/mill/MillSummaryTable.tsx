import { MillSummary } from '../../../types/location';
import { ArrowDown, ArrowUp } from 'lucide-react';

interface MillSummaryTableProps {
  summaries: MillSummary[];
  loading: boolean;
}
...
export default function MillSummaryTable({ summaries, loading }: MillSummaryTableProps) {
  if (loading) {
    return (
  ...
  if (summaries.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm flex items-center justify-center h-32">
        <p className="text-gray-500">No mill data available for the selected period</p>
      </div>
    );
  }
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {summaries.map((summary, index) => {
              ...
              return (
                <tr
                  key={mill?.id || `mill-${index}`}
                  className={`hover:bg-gray-50 ${!mill?.name ? 'bg-yellow-50' : ''
                    }`}
                >                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">
                      {mill?.name || '- No Destination -'}
                    </div>
                  </td>
