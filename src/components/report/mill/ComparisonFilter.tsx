...
interface ComparisonFilterProps {
  mills: Mill[];
  onFilterChange: (startDate: Date | null, endDate: Date | null, millIds: number[]) => void;
}
...
export default function ComparisonFilter({ mills, onFilterChange }: ComparisonFilterProps) {
  const [selectedMills, setSelectedMills] = useState<MillOption[]>([]);
  ...
  const millOptions: MillOption[] = mills
    .filter(mill => mill.id !== undefined)
    .map(mill => ({
      value: mill.id as number,
      label: mill.name,
    }));

  const handleMillChange = (selectedOptions: any) => {
    const options = selectedOptions as MillOption[];
    setSelectedMills(options);
  ...
  const handleDateChange = (ranges: any) => {
    ...
    if (startDate && endDate) {
      onFilterChange(startDate, endDate, selectedMills.map(v => v.value));
      setShowPicker(false);
    }
  };
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm mb-6">
      <div className="grid grid-cols-1 lg:grid-cols-[auto_1fr_auto] gap-4 items-end">
        ...
        <div className="w-full">
          <label className="text-sm font-medium text-gray-700 mb-1 block">Select Destinations to Compare</label>
