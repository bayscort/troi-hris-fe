...
interface ComparisonFilterProps {
  tripTypes: TripType[];
  onFilterChange: (startDate: Date | null, endDate: Date | null, tripTypeIds: number[]) => void;
}
...
export default function ComparisonFilter({ tripTypes, onFilterChange }: ComparisonFilterProps) {
  const [selectedTripTypes, setSelectedTripTypes] = useState<TripTypeOption[]>([]);
  ...
  const tripTypeOptions: TripTypeOption[] = tripTypes
    .filter(tripType => tripType.id !== undefined)
    .map(tripType => ({
      value: tripType.id as number,
      label: tripType.name,
    }));

  const handleTripTypeChange = (selectedOptions: any) => {
    const options = selectedOptions as TripTypeOption[];
    setSelectedTripTypes(options);
  ...
  const handleDateChange = (ranges: any) => {
    ...
    if (startDate && endDate) {
      onFilterChange(startDate, endDate, selectedTripTypes.map(v => v.value));
      setShowPicker(false);
    }
  };
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm mb-6">
      <div className="grid grid-cols-1 lg:grid-cols-[auto_1fr_auto] gap-4 items-end">
        ...
        <div className="w-full">
          <label className="text-sm font-medium text-gray-700 mb-1 block">Select Trip Types to Compare</label>
