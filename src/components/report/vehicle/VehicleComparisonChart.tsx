...
export default function VehicleComparisonChart({ summaries }: VehicleComparisonChartProps) {
  ...
  if (summaries.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-gray-200 bg-muted p-8 h-64 flex items-center justify-center shadow-sm">
        <p className="text-sm text-gray-400 italic">Please select vehicles to compare</p>
      </div>
    );
  }
  ...
  const data = summaries.map((summary) => ({
    name: summary.vehicle.licensePlatNumber,
  ...
  return (
      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
        <div className="mb-4 space-y-2">
          <h2 className="text-base font-semibold text-gray-700">Vehicle Comparison Chart</h2>
