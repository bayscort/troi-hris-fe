...
export default function VehicleSummaryTable({ summaries, loading }: VehicleSummaryTableProps) {
  if (loading) {
    return (
  ...
  if (summaries.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm flex items-center justify-center h-32">
        <p className="text-gray-500">No vehicle data available for the selected period</p>
      </div>
    );
  }
            ...
            {summaries.map((summary, index) => {
              ...
              return (
                <tr
                  ...
                  className={`hover:bg-gray-50 ${!vehicle?.licensePlatNumber ? 'bg-yellow-50' : ''
                    }`}
                >                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="ml-4">
                        <div className="text-sm font-medium text-gray-900">
                          {vehicle?.licensePlatNumber || '- No Vehicle -'}
                        </div>
