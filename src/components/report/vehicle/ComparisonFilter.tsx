...
interface ComparisonFilterProps {
  vehicles: Vehicle[];
  onFilterChange: (startDate: Date | null, endDate: Date | null, vehicleIds: number[]) => void;
}
...
export default function ComparisonFilter({ vehicles, onFilterChange }: ComparisonFilterProps) {
  const [selectedVehicles, setSelectedVehicles] = useState<VehicleOption[]>([]);
  ...
  const vehicleOptions: VehicleOption[] = vehicles
    .filter(vehicle => vehicle.id !== undefined)
    .map(vehicle => ({
      value: vehicle.id as number,
      label: vehicle.licensePlatNumber,
    }));

  const handleVehicleChange = (selectedOptions: any) => {
    const options = selectedOptions as VehicleOption[];
  ...
  const handleDateChange = (ranges: any) => {
    ...
    if (startDate && endDate) {
      onFilterChange(startDate, endDate, selectedVehicles.map(v => v.value));
      setShowPicker(false);
    }
  };
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm mb-6">
      <div className="grid grid-cols-1 lg:grid-cols-[auto_1fr_auto] gap-4 items-end">
        ...
        <div className="w-full">
          <label className="text-sm font-medium text-gray-700 mb-1 block">Select Vehicles to Compare</label>
